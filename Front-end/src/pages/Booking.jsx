...
const PLATE_REGEX_CAR       = /^(\d{2})([A-Z][0-9]|[A-Z]{2}|[A-Z])-(\d{3,5}|\d{3}\.\d{2})$/;

const validatePlate = (plate, vehicleType) => {
  const clean = plate.trim().toUpperCase().replace(/\s/g, '');
  const regex = vehicleType === 'BIKE' ? PLATE_REGEX_MOTORBIKE : PLATE_REGEX_CAR;

  if (!regex.test(clean)) {
    return {
      valid: false,
      message: vehicleType === 'BIKE'
...
export default function Booking() {
  ...
  const checkIfPast = (slotTime) => {
    ...
    if (slotHours === now.getHours() && slotMinutes <= now.getMinutes()) return true;
    return false;
  };

  useEffect(() => {
    if (!date) { setSlotsAvailability({}); return; }
    ...
    const fetchSlotsAvailability = async () => {
      ...
      try {
        ...
        if (machines.length === 0) {
          ...
          setFetchError(`Không tìm thấy máy rửa hoạt động cho ${vehicleType === "BIKE" ? "Xe máy" : "Ô tô"}!`);
          return;
        }
        ...
        if (time && (aggregated[time] === "booked" || checkIfPast(time))) {
          setTime("");
          showToast("Khung giờ đang chọn không còn trống, vui lòng chọn lại!", "error");
        }
      } catch (err) {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      showToast(`Không thể tạo lịch: ${err.response?.data?.message || err.message}`, "error");
  ...
  return (
    <div className="portal-layout-container">
      ...
      <main
        ...
        style={{ display: "flex", flexDirection: "column", flex: 1, padding: "32px 40px 60px", position: "relative", boxSizing: "border-box" }}
      >
        <div>
                ...
                <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
                  ...
                  <div>
                    ...
                    <input
                      ...
                      placeholder={vehicleType === "BIKE" ? "VD: 29K6-44743" : "VD: 51A-123.45"}
                      required
                      className="form-input"
                      style={{ height: "54px", borderColor: plateError ? "#ef4444" : undefined }}
                ...
                <div style={{ display: "flex", flexDirection: "column", gap: "20px" }}>
                  ...
                  <div>
                    <label className="form-label">
                      Giờ rửa xe *{" "}
                      {time && <span style={{ color: "#f97316", textTransform: "none" }}>(Đang chọn: {time})</span>}
                    </label>
                    {!date ? (
                      ...
                      <div className="slots-container">
                        ...
                        <div className="slots-legend">
                          <div className="legend-item"><span className="legend-dot legend-available"></span><span>Trống</span></div>
                          <div className="legend-item"><span className="legend-dot legend-booked"></span><span>Hết máy</span></div>
                          <div className="legend-item"><span className="legend-dot legend-past"></span><span>Đã qua</span></div>
                          <div className="legend-item"><span className="legend-dot legend-selected"></span><span>Đang chọn</span></div>
                        </div>
                      </div>
                    )}
                  </div>
            ...
            <div className="lg:col-span-1">
              <div className="summary-card sticky top-10">
                ...
                <button
                  ...
                  style={{ opacity: isSubmitting || plateError ? 0.7 : 1, cursor: isSubmitting || plateError ? "not-allowed" : "pointer" }}
                >
