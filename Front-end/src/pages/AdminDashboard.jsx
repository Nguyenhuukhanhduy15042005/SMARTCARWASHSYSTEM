...
export default function AdminDashboard() {
  ...
  const fetchData = async () => {
    ...
    try {
      ...
      showToast(
        `Không thể kết nối đến cơ sở dữ liệu: ${errMsg}. Vui lòng kiểm tra Server!`,
        "error",
      );
      setBookings([]);
  ...
  const handleDeleteBooking = async (id) => {
    if (
      !window.confirm(
        "Bạn có chắc chắn muốn xóa vĩnh viễn lịch đặt xe này khỏi cơ sở dữ liệu?",
      )
    ) {
      return;
    }
      ...
      <main className="admin-main-content portal-main-content">
        ...
        <section className="admin-filters-bar">
          ...
          <div className="admin-status-tabs">
            ...
            <button
              className={`admin-tab ${selectedStatus === "3" ? "active" : ""}`}
              onClick={() => setSelectedStatus("3")}
            >
              Đang làm
            </button>
            <button
              className={`admin-tab ${selectedStatus === "4" ? "active" : ""}`}
              onClick={() => setSelectedStatus("4")}
            >
              Hoàn tất
            </button>
            <button
              className={`admin-tab ${selectedStatus === "5" ? "active" : ""}`}
                ...
                >
                  {selectedBooking.status === 1 && (
                    <button
                      ...
                      onClick={() => handleStatusUpdate(selectedBooking.id, 2)}
                    >
                      <i
                  ...
                  {selectedBooking.status === 3 && (
                    <button
                      ...
                      onClick={() => handleStatusUpdate(selectedBooking.id, 4)}
                    >
                      <i
                        className="fa-solid fa-circle-check"
                        style={{ marginRight: "6px" }}
                  ...
                  {selectedBooking.status < 3 && (
                    <button
                      ...
                      onClick={() => handleStatusUpdate(selectedBooking.id, 5)}
                    >
                      <i
